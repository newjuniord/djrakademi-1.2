<script lang="ts">
	...
	const i18n = {
		fr: {
			badge: (n: number) => `Offre groupée · ${n} ressource${n > 1 ? 's' : ''}`,
			courses: (n: number) => `${n} formation${n > 1 ? 's' : ''}`,
			ebooks: (n: number) => `${n} livre${n > 1 ? 's' : ''} électronique${n > 1 ? 's' : ''}`,
			priceLabel: "Prix de l'offre"
		},
		ht: {
			badge: (n: number) => `Pakèt resous · ${n} resous`,
			courses: (n: number) => `${n} fòmasyon`,
			ebooks: (n: number) => `${n} liv dijital`,
			priceLabel: "Pri pakèt la"
		}
	};
...
<a href={getHref(`/bundles/${bundle.id}`)} class="group flex h-full flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:border-amber-400/50 hover:shadow-xl focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-500">
	<div class="relative aspect-[16/10] overflow-hidden bg-zinc-900">
		...
		{/if}
		<div class="absolute left-4 top-4 rounded-full bg-amber-400 px-3 py-1.5 text-[11px] font-black uppercase tracking-wide text-zinc-950">{t.badge(bundle.items.length)}</div>
	</div>
	<div class="flex flex-1 flex-col gap-4 p-5 sm:p-6">
		...
		<div class="flex flex-wrap gap-2 text-[11px] font-bold text-zinc-600">
			{#if courseCount}<span class="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-2.5 py-1"><BookOpen size={13} /> {t.courses(courseCount)}</span>{/if}
			{#if ebookCount}<span class="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-2.5 py-1"><FileText size={13} /> {t.ebooks(ebookCount)}</span>{/if}
		</div>
		<div class="mt-auto flex items-end justify-between gap-3 border-t border-zinc-100 pt-4">
			<div><span class="block text-[11px] font-semibold text-zinc-500">{t.priceLabel}</span><strong class="text-xl font-black text-zinc-950">{formatPublicPrice(bundle.price, bundle.priceUsd)}</strong>{#if useHTG && bundle.originalPrice > bundle.price}<span class="ml-2 text-xs text-zinc-400 line-through">{bundle.originalPrice.toLocaleString('fr-FR')}</span>{/if}</div>
