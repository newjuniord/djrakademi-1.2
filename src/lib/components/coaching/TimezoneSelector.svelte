...
{#if !open}
	<div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-base-300 bg-base-200/50 p-4">
		<div><p class="text-xs text-base-content/50">Fuseau horaire</p><p class="font-medium">{getTimezoneLabel(value)}</p></div>
		<button type="button" class="btn btn-outline btn-sm min-h-10" onclick={() => (open = true)}>Modifier</button>
	</div>
{:else}
	<div class="rounded-lg border border-base-300 bg-base-100 p-3">
		<div class="relative mb-2">
			<Search class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-base-content/40" size={17} />
			<input class="input input-bordered w-full pl-10 pr-10" type="search" placeholder="Rechercher une ville…" bind:value={search} />
			<button class="btn btn-ghost btn-square btn-sm absolute right-1 top-1/2 -translate-y-1/2" type="button" aria-label="Fermer" onclick={() => (open = false)}><X size={17} /></button>
		</div>
		<ul class="menu max-h-56 w-full flex-nowrap overflow-y-auto p-0">
			...
			{:else}<li class="p-3 text-sm text-base-content/55">Aucun résultat.</li>{/each}
		</ul>
	</div>
{/if}
